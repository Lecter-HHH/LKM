#' lm_cor:Batch build linear regression models with specific combinations of covariates.
#'
#'
...
#'
#' @examples
lm_cor <- function(outc,exp,cor,data,only_sig){
  ...
  for(i in 1:length(outc)){
    for(j in 1:length(exp)){
      for(k in 1:length(cor1)){
        ...
        ci <- ci[grep(exp[j], rownames(ci)),]
        result0 <- cbind(result0,ci)
        # 变量更名
        names(result0)[4:6] <- c("p","CI_lower","CI_upper")
        # 生成规范 beta(95%CI)
        result0$effects <- paste(sprintf("%.2f", result0$Estimate)," (",
        ...
        # 提取子集
        result0 <- result0[,c("effects","p","outcome","corname",
